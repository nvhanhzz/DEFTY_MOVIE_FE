...
const PREFIX_CLIENT: string = import.meta.env.VITE_PREFIX_CLIENT as string;

const refreshToken = async (): Promise<boolean> => {
    try {
        const response = await fetch(`${DOMAIN}/${PREFIX_API}/${PREFIX_CLIENT}/${PREFIX_AUTH}/refresh-token`, {
            method: 'POST',
            credentials: 'include',
        });
...
const requestWithRefresh = async (path: string, options: RequestInit): Promise<Response> => {
    const fullPath = `${DOMAIN}/${PREFIX_API}/${PREFIX_CLIENT}/${path}`;
